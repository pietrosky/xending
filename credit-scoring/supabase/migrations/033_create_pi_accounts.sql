...
-- Índices
CREATE INDEX IF NOT EXISTS idx_pi_accounts_active ON pi_accounts (is_active, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pi_accounts_tenant ON pi_accounts (tenant_id);
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'admin_full_access_pi' AND tablename = 'pi_accounts') THEN
    CREATE POLICY "admin_full_access_pi" ON pi_accounts FOR ALL USING (auth.role() = 'admin');
  END IF;
END $$;

-- Broker: solo lectura de cuentas activas
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'broker_read_active_pi' AND tablename = 'pi_accounts') THEN
    CREATE POLICY "broker_read_active_pi" ON pi_accounts FOR SELECT USING (is_active = true);
  END IF;
END $$;

-- Trigger para impedir UPDATE en campos de datos (inmutabilidad)
CREATE OR REPLACE FUNCTION prevent_data_field_update() RETURNS TRIGGER AS $fn$
BEGIN
...
DROP TRIGGER IF EXISTS trg_prevent_data_update ON pi_accounts;
